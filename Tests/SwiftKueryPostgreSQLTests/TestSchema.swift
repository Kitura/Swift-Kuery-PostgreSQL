...
class TestSchema: XCTestCase {
    ...
    func testCreateTable() {
        let t = MyTable()
        let tNew = MyNewTable()
        let tTime = MyTimestampTable(lastUpdated: true, createdAt: true)
            ...
            pool.getConnection() { connection, error in
                ...
                cleanUp(table: t.tableName, connection: connection) { result in
                    cleanUp(table: tNew.tableName, connection: connection) { result in
                        cleanUp(table: tTime.tableName, connection: connection) { result in
                            tTime.create(connection: connection) { result in
